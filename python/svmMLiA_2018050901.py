    ...
    while (iter < maxIter):
        ...
        for i in range(m):
            ...
            if (((labelMat[i] * Ei < -toler) and (alphas[i] < C))
                ...
                if (abs(alphas[j] - alphaJold) < 0.001):
...
def innerL(i, oS):
    ...
    if (((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C))
        ...
        if L == H:
            print "L==H"
            return 0
        eta = 2.0 * oS.K[i, :] - oS.K[i, i] - oS.K[j, j]
        if eta >= 0:
            print "eta>=0"
            return 0
        ...
        b1 = (oS.b - Ei - oS.labelMat[i] *
              (oS.alphas[i] - alphaIold) * oS.K[i, i] - oS.labelMat[j] *
              (oS.alphas[j] - alphaJold) * oS.K[i, j])
        b2 = (oS.b - Ej - oS.labelMat[i] *
              (oS.alphas[i] - alphaIold) * oS.K[i, j] - oS.labelMat[j] *
...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup=('lin', 0)):
    ...
    while ((iter < maxIter) and ((alphaPairsChanged > 0) or (entireSet))):
        ...
        if entireSet:
            ...
            print "fullSet,iter: %d i: %d, pairs chagtend %d " % (
            ...
            for i in nonBoundIs:
                alphaPairsChanged += innerL(i, oS)
                print "non-bound,iter: %d ,i: %d, pairs chagned %d" % (
                    iter, i, alphaPairsChanged)
            iter += 1
        if entireSet:
            entireSet = False
...
def testRbf(k1=1.3):
    dataArr, labelArr = loadDataSet(
        r'D:\Users\gao\Documents\Code\machinelearninginaction\Ch06\testSetRBF.txt'
    )
    ...
    print " there asre %d Supprot Vectores" % shape(sVs)[0]
    m, n = shape(datMat)
    errorCount = 0
    for i in range(m):
    ...
    dataArr, labelArr = loadDataSet(
        r'D:\Users\gao\Documents\Code\machinelearninginaction\Ch06\testSetRBF2.txt'
    )
    errorCount = 0
    dataMat = mat(dataArr)
    ...
    for i in range(m):
        ...
        predict = kernelEval.T * multiply(labelSV, alphas[svInd]) + b
        if sign(predict)!=sign(labelArr[i]): errorCount += 1   
    print " the test error rate is : %f " % (float(erroCount) / m)
