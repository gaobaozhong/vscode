...
def loadDataSet(filename):
    ...
    for line in fr.readlines():
        ...
        dataMat.append([float(lineArr[0]), float(lineArr[1])]) # 可以注意到这里dataMat获得是一个包含两个float的list，而labelMat获得是一个float。
...
def clipAlpha1(aj,H,L):
    if aj >H:
        aj = H
    if aj< L:
        aj = L
    return aj
def clipAlpha2(aj,H,L):
    if aj >H:
        aj = H
    if aj<L:
        aj = L
...
def smoSimple(dataMatIn, classLabel, C, toler, maxIter):
    ...
    print 'mat(dataMatIn):',mat(dataMatIn)
    # !!!
    # bug:
    ...
    print 'mat(classLabel):',mat(classLabel)
    print 'mat(classLabel).transpose()',mat(classLabel).transpose()
    ...
    print 'alphas = mat(zeros((m, 1))):',mat(zeros((m, 1)))
...
#         print "iteration number: %d" % iter
#     return b,alphas
# def smoSimple3(dataMatIn,classLabel,C,toler,maxIter):
#     dataMatrix =
#     labelMat =
#     b =
#     m,n=
...
def kernelTrans(X,A,kTup):
    ...
    if KTUp[0] == 'lin': 
        ...
        K = exp(K / (-1 * kTup[1] ** 2))
    else: raise NameError('Houston we have a problem -- that kernel is not recoignized')
    return K
